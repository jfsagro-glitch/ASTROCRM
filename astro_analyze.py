...
def parse_utc_offset(value):
    ...
    if direction == "w": offset = -offset
    return offset

def extract_location_from_infobox(infobox_raw):
    """Extract lat/lon/utc from infobox_json"""
    infobox = {}
    if infobox_raw:
        try:
...
def run_analysis(db_path, limit=5000, rating_filter='AA,A,B', output_file=None):
    ...
    for i, row in enumerate(rows):
        ...
        # Extract coordinates from infobox_json
        lat, lon_coord, utc = extract_location_from_infobox(row['infobox_json'])
        ...
        # Check calendar type from infobox
        use_julian = False
        if row['infobox_json']:
            try:
                ib = json.loads(row['infobox_json'])
                use_julian = ib.get('ccalendar', 'g') == 'j'
            except:
                pass

        try:
            JD = jd(yr, mo, dy, h-utc, mi, sc, julian=use_julian)
            sun_lon = sun(JD)[0]
    ...
    for row in rows2[:min(5000,len(rows2))]:
        ...
        h, mi, sc = time_parsed
        lat, lon_coord, utc = extract_location_from_infobox(row['infobox_json'])
        if lat is None or lon_coord is None: continue
        if utc is None: utc = 0
