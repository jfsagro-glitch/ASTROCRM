...
def _person_summary(overview: Dict[str, Any], centers: List[Dict[str, Any]], channels: List[Dict[str, Any]], gates: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    identity = (
        ...
        "Главная особенность: вы раскрываетесь через свою уникальную стратегию.\n\n"
        "Просто: у вас есть врожденный дизайн, который показывает, как лучше всего принимать решения и взаимодействовать с жизнью.\n"
        "Что это значит: некоторые люди интуитивны, другие аналитичны, третьи действуют через ощущения — нет лучше или хуже, это просто ваш тип.\n"
        "Что делать: изучите свой тип и стратегию, используйте их как компас в решениях, и вы заметите, как жизнь начнет складываться легче."
    )
    decision = (
        ...
        "Просто: ваше тело и интуиция знают правильный ответ лучше, чем логика.\n"
        "Что это значит: когда вы слышите свой внутренний авторитет, дела идут гладко; когда игнорируете — начинаются проблемы и усталость.\n"
        "Что делать: перед важным решением дайте себе паузу, проверьте внутренний сигнал, прислушайтесь к телу и только потом действуйте."
    )
    strengths = (
        ...
        + ".\n\n"
        "Просто: именно в этих темах у вас легче всего получаются результаты и вы чувствуете прилив энергии.\n"
        "Что это значит: это ваши природные таланты, которыми вы можете опираться даже в сложные периоды.\n"
        "Что делать: выбирайте проекты и задачи, где требуются именно эти качества — тогда вы будете в состоянии потока."
    )
    risks = (
        ...
        + ".\n\n"
        "Просто: открытые центры — это места, где вы впитываете энергию других людей, как губка.\n"
        "Что это значит: в стрессе можно торопиться, обещать лишнее, брать чужие проблемы на себя и уставать быстрее, чем нужно.\n"
        "Что делать: распознавайте, когда включается не-свой паттерн; отступайте и даете себе время переосмыслить; выбирайте окружение, где вы чувствуете себя спокойнее."
    )
    recommendations = (
        ...
        "Просто: это практические шаги, которые помогут вам жить в согласии со своим дизайном.\n"
        "Что это значит: каждый день и каждое решение — это шанс практиковать свою стратегию и авторитет.\n"
        "Что делать: начните с одного шага; может быть, просто начните замечать сигналы своего тела в течение недели."
    )
    return {
        "identity": identity,
...
def _reader_mode_expanded_text(cross: Dict[str, Any], profile_context_ru: Optional[str], base_text: str) -> str:
    cross_title = cross.get("cross_name_ru") or cross.get("name") or "Ваш инкарнационный крест"
    short_theme = cross.get("interpretation_short_ru") or base_text
    long_theme = cross.get("interpretation_long_ru") or ""
    life_theme = cross.get("life_theme_ru") or ""
    career_theme = cross.get("career_theme_ru") or ""
    relationship_theme = cross.get("relationship_theme_ru") or ""
    shadow_theme = cross.get("shadow_theme_ru") or ""

    return (
        f"{short_theme}\n\n"
        f"Кто вы по системе\n"
        f"Ваш базовый жизненный вектор в рамках креста «{cross_title}». "
        f"Это не жесткий приговор, а рабочая карта: что усиливать, что стабилизировать, где не идти против своей природы.\n\n"
        f"Суть креста и его потенциал\n"
        f"{long_theme or short_theme} "
        f"Этот крест дает потенциал для устойчивой реализации через повторяемые решения и верный ритм. "
        f"Чем яснее вы выбираете приоритеты, тем быстрее раскрывается ваш реальный масштаб.\n\n"
        f"Что крест требует от вас\n"
        f"1) Не спешить с решениями в стрессе. 2) Строить путь шагами, а не рывками. "
        f"3) Регулярно сверяться: что дает энергию, а что забирает. "
        f"4) Брать только те обязательства, которые телу и психике реально по силам.\n\n"
        f"Плюсы и возможности\n"
        f"- Личное ядро реализации: {life_theme or 'выход в свои сильные стороны через практику и дисциплину.'}\n"
        f"- Карьера и дело: {career_theme or 'рост через системность, качество решений и накопление компетенций.'}\n"
        f"- Отношения и связи: {relationship_theme or 'усиление через честные договоренности и уважение границ.'}\n\n"
        f"Риски и слабые места\n"
        f"{shadow_theme or 'Главный риск: действия из тревоги, спешки или желания срочно доказать ценность. Это ведет к выгоранию и потере фокуса.'}\n\n"
        f"Как избежать ошибок и что можно исправить\n"
        f"- Перед важным решением: пауза, проверка состояния, затем действие.\n"
        f"- Если уже ушли в перегруз: уменьшите количество задач до 1-2 ключевых и восстановите ритм сна/питания.\n"
        f"- Если появились конфликты: переходите с оценок на факты, ожидания и конкретные договоренности.\n"
        f"- Если упала мотивация: возвращайтесь к маленьким завершенным шагам и телесной дисциплине.\n\n"
        f"Питание и энергия\n"
        f"Лучше работает простая схема: стабильные приемы пищи, вода, достаточный сон, регулярное движение без перегибов. "
        f"В эмоционально нестабильные дни не принимайте резких решений о режиме: сначала восстановление, потом коррекция.\n\n"
        f"Локации и среда\n"
        f"Выбирайте пространство, где есть ощущение спокойствия, понятный ритм и минимум токсичного давления. "
        f"Если место «не ваше», реализация обычно замедляется. Полезен тест локации на 2-3 дня: сон, энергия, концентрация, настроение.\n\n"
        f"Личные отношения\n"
        f"Опора отношений для вашего креста: ясные границы, честная коммуникация и проверяемые договоренности. "
        f"Не обещайте на эмоциональном пике. Лучше меньше обещаний, но больше устойчивого выполнения.\n\n"
        f"Компенсаторики по сферам жизни\n"
        f"- Работа: недельный фокус на 1 главной задаче + ежедневный короткий чек-ин прогресса.\n"
        f"- Деньги: правило двух шагов — сначала проверка ресурса, потом финансовое решение.\n"
        f"- Здоровье: сон и восстановление ставить в календарь как обязательство, а не опцию.\n"
        f"- Отношения: еженедельная сверка ожиданий и роли каждого в паре/семье.\n"
        f"- Самореализация: двигаться через повторяемую практику, а не через эмоциональные рывки.\n\n"
        f"Прогноз по периодам: как использовать\n"
        f"Солнечные периоды используйте для стратегических шагов (план, запуск, закрепление). "
        f"Лунные окна — для тактики дня (переговоры, встречи, решения на ближайшие 24 часа). "
        f"В каждом периоде задавайте три вопроса: где возможность, где риск, какое одно действие даст наилучший результат."
        + (f"\n\nПояснение по профилю\n{profile_context_ru}" if profile_context_ru else "")
    )
