...
const PA_PLANETS: Record<string, { name: string; ru: string; func: string; key: string; color: string }> = {
  sun:      { name: 'Sun',     ru: 'Солнце',    color: '#d4a853', key: '☉',
    func: '«Я-сам» — центр воли и намерения, отцовский принцип. Где Солнце — там главная сцена жизни.' },
  moon:     { name: 'Moon',    ru: 'Луна',       color: '#9ab5d4', key: '☽',
    func: '«Чувствовалка» — эмоции, инстинкты, материнский принцип. Показывает потребности и реакции.' },
  mercury:  { name: 'Mercury', ru: 'Меркурий',   color: '#88c4a8', key: '☿',
    func: '«Думалка» — речь, коммуникация, анализ. Отвечает за восприятие информации и связи.' },
  venus:    { name: 'Venus',   ru: 'Венера',     color: '#d48aaa', key: '♀',
    func: '«Выбиралка» — любовь, деньги, вкус. Показывает, что притягивает и чем дорожит человек.' },
  mars:     { name: 'Mars',    ru: 'Марс',       color: '#d45b5b', key: '♂',
    func: '«Делалка» — действие, воля, преодоление. Двигатель и источник энергии для любых дел.' },
  jupiter:  { name: 'Jupiter', ru: 'Юпитер',    color: '#d4a04a', key: '♃',
    func: '«Расширитель» — философия, рост, экспертность. Зона щедрости, удачи и мировоззрения.' },
  saturn:   { name: 'Saturn',  ru: 'Сатурн',    color: '#8899bb', key: '♄',
    func: '«Концентратор» — труд, структура, иерархия. Через ограничения даёт власть и результат.' },
  uranus:   { name: 'Uranus',  ru: 'Уран',      color: '#5bbbcc', key: '⛢',
    func: '«Ускоритель» — прорыв, технологии, сверхразум. Место неожиданных переворотов и открытий.' },
  neptune:  { name: 'Neptune', ru: 'Нептун',    color: '#7788dd', key: '♆',
    func: '«Сверхчуйка» — интуиция, духовность, иллюзии. Растворяет границы и указывает на идеалы.' },
  pluto:    { name: 'Pluto',   ru: 'Плутон',    color: '#bb77aa', key: '♇',
    func: '«Сверхволя» — трансформация, власть, кризис. Разрушает старое, чтобы родилось новое.' },
  node:     { name: 'N.Node',  ru: 'С.Узел',    color: '#ccaa44', key: '☊',
    func: 'Точка кармической эволюции — направление роста в этой жизни.' },
  chiron:   { name: 'Chiron',  ru: 'Хирон',     color: '#66aabb', key: '⚷',
    func: '«Раненый целитель» — исцеление через боль и принятие. Глубинная рана и дар.' },
  lilith:   { name: 'Lilith',  ru: 'Лилит',     color: '#9966aa', key: '⚸',
    func: '«Тёмная сила» — табу, теневая сторона, запретные желания. Место силы через тень.' },
};

const PA_HOUSES: Record<number, { name: string; theme: string; detail: string }> = {
  1:  { name: 'Дом Я',             theme: 'Тело, маска, бренд личности',
        detail: 'Первое впечатление, внешность, стиль самопрезентации. Как человека видят другие.' },
  2:  { name: 'Дом ресурса',       theme: 'Личные деньги, имущество, иммунитет',
        detail: 'Способность зарабатывать и копить. Ценности, за которые человек готов платить.' },
  3:  { name: 'Дом ближнего',      theme: 'Братья, соседи, транспорт, соцсети',
        detail: 'Ближнее окружение, короткие поездки, информация, коммуникации в быту.' },
  4:  { name: 'Дом рода',          theme: 'Семья, корни, родина, недвижимость',
        detail: 'Психологическая база, отношения с родителями (особенно матерью), эгрегор рода.' },
  5:  { name: 'Дом творчества',    theme: 'Дети, романы, риск, самопроявление',
        detail: 'Казино-церковь: азарт, любовные романы, творческие проекты, дети как продолжение.' },
  6:  { name: 'Дом службы',        theme: 'Наёмный труд, здоровье, коллеги, рутина',
        detail: 'Ежедневные обязанности, тело как инструмент, отношения с коллегами и подчинёнными.' },
  7:  { name: 'Дом партнёра',      theme: 'Брак, договоры, суды, открытые враги',
        detail: 'Всё «не-я»: партнёры, соперники, союзы. Качество всех значимых отношений.' },
  8:  { name: 'Дом трансформации', theme: 'Чужие ресурсы, кризисы, эрос, налоги',
        detail: 'Глубинные изменения, сексуальность, наследство, кризисы как точки роста.' },
  9:  { name: 'Дом экспертизы',    theme: 'Высшее знание, заграница, мировоззрение',
        detail: 'Философия, религия, дальние поездки, издательство, судебные разбирательства.' },
  10: { name: 'Дом статуса',       theme: 'Карьера, репутация, иерархия, государство',
        detail: 'Публичный образ, профессиональные достижения, отношения с властью и отцом.' },
  11: { name: 'Дом аудитории',     theme: 'Единомышленники, IT, планы, будущее',
        detail: 'Сообщество, мечты и цели, большая аудитория, нетворкинг, технологии.' },
  12: { name: 'Дом тайного',       theme: 'Изоляция, хронические темы, творчество в тиши',
        detail: 'Скрытые враги, кармические долги, монастырь/тюрьма/больница, эмиграция.' },
};
...
const PA_SIGNS: Record<string, { ru: string; element: string; mode: string; keyword: string; desc: string }> = {
  aries:       { ru: 'Овен',        element: 'Огонь', mode: 'Кардинальный', keyword: 'Инициатива',
    desc: 'Прямолинейный, первопроходец, импульсивный лидер. Действует раньше, чем думает.' },
  taurus:      { ru: 'Телец',       element: 'Земля', mode: 'Фиксированный', keyword: 'Ресурс',
    desc: 'Терпеливый накопитель ресурсов. Ценит стабильность, красоту и чувственные удовольствия.' },
  gemini:      { ru: 'Близнецы',    element: 'Воздух', mode: 'Мутабельный', keyword: 'Коммуникация',
    desc: 'Любопытный многогранник. Обрабатывает множество потоков информации одновременно.' },
  cancer:      { ru: 'Рак',         element: 'Вода', mode: 'Кардинальный', keyword: 'Защита',
    desc: 'Эмоционально чуткий, защищает близких. Живёт памятью и интуицией.' },
  leo:         { ru: 'Лев',         element: 'Огонь', mode: 'Фиксированный', keyword: 'Творчество',
    desc: 'Щедрый монарх, любит центр сцены. Создаёт и вдохновляет, требует признания.' },
  virgo:       { ru: 'Дева',        element: 'Земля', mode: 'Мутабельный', keyword: 'Анализ',
    desc: 'Мастер детализации и совершенствования. Видит дефекты там, где другие не замечают.' },
  libra:       { ru: 'Весы',        element: 'Воздух', mode: 'Кардинальный', keyword: 'Баланс',
    desc: 'Дипломат и эстет. Ищет гармонию, взвешивает любое решение, партнёрски ориентирован.' },
  scorpio:     { ru: 'Скорпион',    element: 'Вода', mode: 'Фиксированный', keyword: 'Трансформация',
    desc: 'Глубокий исследователь тайн. Умеет трансформировать кризисы в силу.' },
  sagittarius: { ru: 'Стрелец',     element: 'Огонь', mode: 'Мутабельный', keyword: 'Расширение',
    desc: 'Философ и путешественник. Ищет смысл и расширяет горизонты через опыт и знание.' },
  capricorn:   { ru: 'Козерог',     element: 'Земля', mode: 'Кардинальный', keyword: 'Структура',
    desc: 'Дисциплинированный строитель. Медленно карабкается к вершине, но добирается.' },
  aquarius:    { ru: 'Водолей',     element: 'Воздух', mode: 'Фиксированный', keyword: 'Инновация',
    desc: 'Революционер-гуманист. Мыслит системно, живёт будущим, ценит независимость.' },
  pisces:      { ru: 'Рыбы',        element: 'Вода', mode: 'Мутабельный', keyword: 'Растворение',
    desc: 'Медиум и мечтатель. Растворяет границы, улавливает тонкие вибрации, сострадателен.' },
};
...
function PlanetRow({ name, p, dignityStr, isDark }: {
  ...
  return (
    <div style={{
      ...
      {/* Interpretation */}
      <p style={{ margin: 0, fontSize: 13, color: isDark ? '#cbd5e1' : '#374151', lineHeight: 1.6 }}>
...
function PatternsBlock({ patterns }: { patterns: Record<string, boolean> }) {
  ...
  if (!active.length) return <p style={{ color: '#64748b', fontSize: 13 }}>Значимые конфигурации не обнаружены.</p>;
  return (
    <div>
      {active.map(([key]) => {
    ...
    <div style={{ color: textColor, fontFamily: 'system-ui, sans-serif', maxWidth: 860, margin: '0 auto' }}>
      ...
      <Section title="Доминанты карты" icon="🌿" accent="#34d399">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {/* Elements */}
          <div>
            <p style={{ margin: '0 0 8px', fontSize: 12, color: '#94a3b8', fontWeight: 600 }}>СТИХИИ</p>
            ...
            <p style={{ fontSize: 12, color: isDark ? '#94a3b8' : '#6b7280', marginTop: 8, lineHeight: 1.5 }}>
              {ELEMENT_DESC[dom.topElement]}
            </p>
          </div>
          {/* Modes + house types */}
          <div>
            <p style={{ margin: '0 0 8px', fontSize: 12, color: '#94a3b8', fontWeight: 600 }}>МОДУСЫ</p>
            ...
            <p style={{ margin: '10px 0 4px', fontSize: 12, color: '#94a3b8', fontWeight: 600 }}>ДОМА</p>
            <p style={{ fontSize: 12, color: isDark ? '#cbd5e1' : '#374151', lineHeight: 1.5 }}>
            ...
            {dom.houses.angular >= 5 && <p style={{ fontSize: 11, color: '#fbbf24', margin: '4px 0 0' }}>
      ...
      <Section title="Кармическая ось (Узлы)" icon="🔮" accent="#c084fc">
        {(() => {
          ...
          if (!node) return <p style={{ color: '#64748b', fontSize: 13 }}>Данные узлов отсутствуют.</p>;
        ...
        <Section title="Лунная фаза рождения" icon="🌙" accent="#9ab5d4">
          <div style={{ fontSize: 13, color: isDark ? '#cbd5e1' : '#374151', lineHeight: 1.7 }}>
            ...
            {(() => {
              ...
              if (angle < 45)  return <p>Новолуние — «семя»: врождённый инстинкт начинать, инициировать, творить с нуля.</p>;
              if (angle < 90)  return <p>Молодая Луна — «росток»: импульс к проявлению, первые шаги, энтузиазм без полного опыта.</p>;
              if (angle < 135) return <p>Первая четверть — «кризис действия»: натура, реализующая себя через преодоление препятствий.</p>;
              if (angle < 180) return <p>Горбатая Луна — «совершенствование»: аналитический ум, стремление к мастерству и завершённости.</p>;
              if (angle < 225) return <p>Полнолуние — «плод»: объективность, осознанность, жизнь ради других, публичность.</p>;
              if (angle < 270) return <p>Убывающая горбатая Луна — «дистилляция»: философ-учитель, распространяет знание и опыт.</p>;
              if (angle < 315) return <p>Последняя четверть — «переоценка»: переосмысляет прошлое, реформирует структуры.</p>;
              return <p>Бальзамическая Луна — «семя будущего»: медиум, пророк, завершает циклы, несёт карму прошлого поколения.</p>;
            })()}
          </div>
        </Section>
      )}
