...
const PA_SIGNS: Record<string, { ru: string; element: string; mode: string; keyword: string; desc: string }> = {
  aries:       { ru: 'Овен',        element: 'Огонь', mode: 'Кардинальный', keyword: 'Инициатива',
    desc: 'Первый и бесстрашный. Овен не ждёт разрешения — он просто идёт вперёд. Иногда обжигается, но именно он прокладывает тропу для остальных.' },
  taurus:      { ru: 'Телец',       element: 'Земля', mode: 'Фиксированный', keyword: 'Ресурс',
    desc: 'Терпеливый и чувственный. Телец строит медленно, но так прочно, что это стоит на века. Знает толк в настоящих удовольствиях и умеет ценить момент.' },
