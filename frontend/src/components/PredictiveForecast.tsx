...
export default function PredictiveForecast({ startStr, endStr, isDark }: Props) {
  ...
  const SummaryTab = () => {
    ...
    // Sphere narratives
    const SPHERE_ADVICE: Record<string, [string, string]> = {
      health:     ['💚 Здоровье', 'Период благоприятен для физических нагрузок, профилактики и восстановления.'],
      work:       ['💼 Работа', 'Хорошее время для карьерных инициатив, переговоров и долгосрочных проектов.'],
      love:       ['❤️ Личная жизнь', 'Эмоциональный фон благоприятен для углубления отношений и романтики.'],
      energy:     ['⚡ Энергия', 'Высокий витальный потенциал — используйте для старта новых дел.'],
      finance:    ['💰 Финансы', 'Период подходит для финансового планирования, инвестиций и переговоров.'],
      creativity: ['🎨 Творчество', 'Вдохновение сильно — создавайте, экспериментируйте, выражайте себя.'],
      spirit:     ['✨ Духовность', 'Время для медитации, саморефлексии и духовных практик.'],
    };
        ...
        <div className={`rounded-xl border ${card} p-4`}>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            <div className={`p-2.5 rounded-lg ${isDark ? 'bg-green-500/10 border border-green-500/20' : 'bg-green-50 border border-green-200'}`}>
              ...
              <p className={`text-[10px] ${text} mt-0.5 leading-relaxed`}>{SPHERE_ADVICE[bestSph.id]?.[1]}</p>
            </div>
            <div className={`p-2.5 rounded-lg ${isDark ? 'bg-orange-500/10 border border-orange-500/20' : 'bg-orange-50 border border-orange-200'}`}>
              ...
              <p className={`text-[10px] ${text} mt-0.5 leading-relaxed`}>Уделите особое внимание этой сфере в период.</p>
            </div>
          </div>
        </div>
